...
void Bullet::init(const glm::ivec2 &tileMapPos, const glm::vec2 &pos, const glm::vec2 &dir, int speed, ShaderProgram &shaderProgram)
{
	...
	sprite->setAnimationSpeed(DESTROYED, 8);
	sprite->addKeyframe(DESTROYED, glm::vec2(3.f / 4.f, 0.f));
	sprite->addKeyframe(DESTROYED, glm::vec2(0.f / 4.f, 1.f / 4.f));
...
void Bullet::changeBulletAnimation(const int animNum) const
{
	sprite->changeAnimation(animNum);
}

int Bullet::getAnimation() const
{
